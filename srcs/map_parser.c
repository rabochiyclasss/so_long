...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/31 19:32:08 by student           #+#    #+#             */
/*   Updated: 2025/07/31 19:32:20 by student          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void validate_content(t_game *game)
{
    int y;
    int x;
    int exit_count = 0;
    ...
    if (exit_count != 1)
        ft_error("Map must have exactly one exit");
    if (player_count != 1)
        ft_error("Map must have exactly one player");
...
void validate_path(t_game *game)
{
    ...
    if (!found_player)
    {
        ...
        ft_error("Player not found in map copy");
    }
    ...
    {
        free_map(map_copy, game->map_height);
        ft_error("Some collectibles are not reachable");
    }
    
    if (!check_exit_reachable(map_copy, game))
    {
        free_map(map_copy, game->map_height);
        ft_error("Exit is not reachable");
    }
...
void parse_map(char *file, t_game *game)
{
    ...
    if (i != game->map_height)
        ft_error("Map reading inconsistency");
